...
echo "Running: python3 -u /rtl_433_mqtt_hass.py -r -d -u ${MQTT_USERNAME} -P ${MQTT_PASSWORD} -H ${MQTT_HOST} -p ${MQTT_PORT} -D ${DISCOVERY_PREFIX} -R ${RTL_TOPIC} -i ${DISCOVERY_INTERVAL} ${OTHER_ARGS}"
python3 -u /rtl_433_mqtt_hass.py -r -d -u ${MQTT_USERNAME} -P ${MQTT_PASSWORD} -H ${MQTT_HOST} -p ${MQTT_PORT} -D ${DISCOVERY_PREFIX} -R ${RTL_TOPIC} -i $DISCOVERY_INTERVAL $OTHER_ARGS
